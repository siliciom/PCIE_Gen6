...
class PCIe_RC_phy_driver extends uvm_driver #(PCIe_sequence_item);
   ...
   task run_phase(uvm_phase phase);
    ...
    task process_sequence_items();
	forever begin
          // Wait for LTSSM/PL model to request receiver detection
          `uvm_info("RC_PHY","Waiting for TxDetectRx...",UVM_LOW)
          wait(rc_pipe_intf_tx.tx_detect_rx == 1'b1);
          `uvm_info("RC_PHY","TxDetectRx detected - Receiver Detection Started",UVM_LOW)
        repeat(5)
              @(posedge rc_pipe_intf_tx.pclk);
              ep_present = ep_pl_model.ep_present;
              `uvm_info("RC_PHY_DRIVER",$sformatf("RC_PHY_reads_EP_PRESENT = %0d", ep_present),UVM_LOW)
           // Receiver detection result
           rc_pipe_intf_tx.phy_status <= 1'b1;
           if (ep_pl_model.no_receiver_test) begin
              rc_pipe_intf_tx.rx_status <= 3'b000;
              `uvm_info("RC_PHY","RC_No_Receiver_Test: forcing receiver-detection FAIL (rx_status=000)",UVM_LOW)
           end
           else if (ep_present)
              rc_pipe_intf_tx.rx_status <= 3'b011;
           else
              rc_pipe_intf_tx.rx_status <= 3'b000;
          `uvm_info("RC_PHY","PHY receiver detection completed",UVM_LOW)
          @(posedge rc_pipe_intf_tx.pclk);
           rc_pipe_intf_tx.phy_status   <= 1'b0;
           rc_pipe_intf_tx.rx_status    <= 3'b000;
           rc_pipe_intf_tx.tx_detect_rx <= 1'b0;
          seq_item_port.get_next_item(pcie_seq_item);
          `uvm_info("RC_PHY","RC PHY SEQUENCE ITEM RECEIVED",UVM_LOW)
           seq_item_port.item_done();
          `uvm_info("RC_PHY","RC PHY SEQUENCE ITEM COMPLETED",UVM_LOW)
       end
    endtask
       ...
       begin
        @(negedge rc_pipe_intf_tx.pclk);
          ...
          if(tx_valid)begin
           ...
           `uvm_info("RC_PHY_DRIVER",$sformatf("Received_Data = %08h Queue_Size = %0d",tx_data,rc_data_q.size()),UVM_LOW)
...
task tx_piso(input bit [`PCIe_MON_DATA_W-1:0] data_in);
        bit piso_data_out;
