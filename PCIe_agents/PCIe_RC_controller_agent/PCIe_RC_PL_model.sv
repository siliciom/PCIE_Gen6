...
* you agree to and are bound by the terms of the SILICIOM TECHNOLOGIES PVT LTD license agreement.
...
class PCIe_RC_PL_model extends uvm_component;
  ...
  task scramble(
    ...
    bit scramble_bit;
    data_out = data_in;
    `uvm_info("PCIe_PL_MODEL",
              "ENTERED_INTO_SCRAMBLER_TASK",
              UVM_LOW)
    ...
    `uvm_info("PCIe_PL_MODEL",
              "EXIT_FROM_SCRAMBLER_TASK",
              UVM_LOW)
  endtask
  //----------------------------------------------------------------------------
  // Scramble one PIPE word
  //----------------------------------------------------------------------------
  task scramble_32(
    ...
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("SCRAMBLE_32_INPUT = %08h", data_in),
              UVM_LOW)
    for(int i = 0; i < `PCIe_PL_PIPE_WORD_W; i += `PCIe_BYTE_W)
    begin
      scramble(
        data_in[i +: `PCIe_BYTE_W],
        temp
      );
      data_out[i +: `PCIe_BYTE_W] = temp;
    end
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("SCRAMBLE_32_OUTPUT = %08h", data_out),
              UVM_LOW)
  endtask
  ...
  task tx_gray_encode(
    ...
    gray_out = '0;
    `uvm_info("PCIe_PL_MODEL",
              "ENTERED_INTO_GRAY_ENCODE_TASK",
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("GRAY_ENCODE_INPUT = %0b", data_in),
              UVM_LOW)
    for(int i = 0;
        i < `PCIe_PL_PIPE_WORD_W;
        i = i + `PCIe_PL_SYMBOL_W)
    begin
      ...
      case(symbol)
        ...
        default:
          gray_out[i +: `PCIe_PL_SYMBOL_W] = '0;
      endcase
    end
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("GRAY_ENCODE_OUTPUT = %0b",gray_out),
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              "EXIT_FROM_GRAY_ENCODE_TASK",
              UVM_LOW)
  endtask
  ...
  task tx_parity_generate(
    ...
    `uvm_info("PCIe_PL_MODEL",
              "ENTERED_INTO_PARITY_GENERATE_TASK",
              UVM_LOW)
    parity_bit = ^data_in;
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("PARITY_INPUT = %08h PARITY = %0b",
                        data_in,
                        parity_bit),
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("PARITY_GENERATE_OUTPUT =%b",parity_bit),
              UVM_LOW)
  endtask
  //----------------------------------------------------------------------------
  // TX precoder
  ...
  task tx_precoder(
    ...
    precoded_data = '0;
    `uvm_info("PCIe_PL_MODEL",
              "ENTERED_INTO_PRE_ENCODE_TASK",
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("PRECODE_INPUT = %0b",gray_data),
              UVM_LOW)
    for(int i = 0;
        i < `PCIe_PL_PIPE_WORD_W;
        i = i + `PCIe_PL_SYMBOL_W)
    begin
      ...
      precoded_data[i +: `PCIe_PL_SYMBOL_W] =
          current_symbol ^ previous_symbol;
      previous_symbol =
          precoded_data[i +: `PCIe_PL_SYMBOL_W];
    end
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("PRECODE_OUTPUT = %0b",precoded_data),
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              "EXIT_FROM_PRE_ENCODE_TASK",
              UVM_LOW)
  endtask
  ...
  task tx_process(
    ...
    bit                            parity;
    `uvm_info("PCIe_PL_MODEL",
              "ENTERED_INTO_TX_PROCESS_TASK",
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("The data_in_from PL inside tx_process is %d",
                        data_in),
              UVM_LOW)
    case(mode)
      //------------------------------------------------------------------------
      ...
      begin
        ...
        tx_parity_q.push_back(parity);
        `uvm_info("PCIe_PL_MODEL",
                  $sformatf("TX_PARITY = %0b PARITY_QUEUE_SIZE = %0d",
                            parity,
                            tx_parity_q.size()),
                  UVM_LOW)
        tx_precoder(gray_data, pre_data);
    ...
    endcase
    `uvm_info("PCIe_PL_MODEL",
              $sformatf("TX_PROCESS_INPUT=%08h OUTPUT=%08h",
                        data_in,
                        data_out),
              UVM_LOW)
    `uvm_info("PCIe_PL_MODEL",
              "EXIT_FROM_TX_PROCESS_TASK",
              UVM_LOW)
  endtask
  //----------------------------------------------------------------------------
  // PL model receives the DLP result
  //----------------------------------------------------------------------------
  function void write(PCIe_sequence_item item);
    ...
    pl_qu.delete();
    for(int i = 0;
        i < `PCIe_DLP_FLIT_BYTE_W;
        i += `PCIe_PL_BYTES_PER_WORD)
    begin
      temp = '0;
      for(int j = 0;
          j < `PCIe_PL_BYTES_PER_WORD;
          j++)
      begin
        if((i+j) < `PCIe_DLP_FLIT_BYTE_W)
        begin
          temp[j*`PCIe_BYTE_W +: `PCIe_BYTE_W] =
              dl_flit_out[i+j];
        end
      end
