...
task rc_state_config_complete();
    ...
    /*task send_ordered_set(input bit [7:0] ordered_set [0:15],input int count,input string os_name = "ORDERED_SET");
        for (int os_count = 0; os_count < count; os_count++) begin
            `uvm_info("RC_LTSSM",$sformatf("Sending %s %0d/%0d",os_name, os_count+1, count),UVM_LOW)
            rc_pipe_intf_tx.tx_data <= {ordered_set[3],ordered_set[2],ordered_set[1],ordered_set[0]};
            rc_pipe_intf_tx.tx_valid <= 1'b1;
            @(posedge rc_pipe_intf_tx.pclk);
            rc_pipe_intf_tx.tx_data <= {ordered_set[7],ordered_set[6],ordered_set[5],ordered_set[4]};
            @(posedge rc_pipe_intf_tx.pclk);
            rc_pipe_intf_tx.tx_data <= {ordered_set[11],ordered_set[10],ordered_set[9],ordered_set[8]};
            @(posedge rc_pipe_intf_tx.pclk);
            rc_pipe_intf_tx.tx_data <= {ordered_set[15],ordered_set[14],ordered_set[13],ordered_set[12]};
            @(posedge rc_pipe_intf_tx.pclk);
        end
        // Stop transmission
        rc_pipe_intf_tx.tx_data       <= 32'h00000000;
        rc_pipe_intf_tx.tx_valid      <= 1'b0;
        `uvm_info("RC_LTSSM",$sformatf("%s TRANSMISSION_COMPLETED : %0d ORDERED SETS",os_name,count),UVM_LOW)
    endtask*/
...
task send_ordered_set(input bit [7:0] ordered_set [`PCIe_TS_OS_SIZE], input int count, input string os_name = "ORDERED_SET");
        ...
        for (int os_count = 0; os_count < count; os_count++) begin
           `uvm_info("RC_LTSSM",$sformatf("Sending %s %0d/%0d",os_name, os_count+1, count),UVM_LOW)
           for (int dword = 0; dword < `PCIe_PL_BYTES_PER_WORD; dword++) begin
              raw_data = {ordered_set[dword*`PCIe_PL_BYTES_PER_WORD+3], ordered_set[dword*`PCIe_PL_BYTES_PER_WORD+2],ordered_set[dword*`PCIe_PL_BYTES_PER_WORD+1], ordered_set[dword*`PCIe_PL_BYTES_PER_WORD+0]};
              `uvm_info("RC_LTSSM", $sformatf("%s DWORD%0d RAW_DATA = %08h", os_name, dword, raw_data), UVM_LOW)
              // OS symbol 0, 8, 15 -> BYPASS SCRAMBLING ONLY
              if (os_count == `PCIe_OS_BYPASS_IDX_0 || os_count == `PCIe_OS_BYPASS_IDX_1 || os_count == `PCIe_OS_BYPASS_IDX_2) begin
                 scramble_data = raw_data;
                 `uvm_info("RC_LTSSM",$sformatf("%s OS=%0d DWORD%0d -> SCRAMBLE_BYPASSED SCRAMBLE_DATA=%08h",os_name, os_count, dword, scramble_data), UVM_LOW)
              end
              else begin
                 scramble_32(raw_data, scramble_data);
                 `uvm_info("RC_LTSSM",$sformatf("%s OS=%0d DWORD%0d -> SCRAMBLED SCRAMBLE_DATA=%08h",os_name, os_count, dword, scramble_data), UVM_LOW)
              end
              ...
              `uvm_info("RC_LTSSM", $sformatf("%s DWORD%0d PIPE_DATA = %08h", os_name, dword, pre_data), UVM_LOW)
...
task reset_scrambler();
   ...
   function void write(PCIe_sequence_item item);
     `uvm_info("RC_PL_MODEL",
              $sformatf("PL -> INTERFACE received 242-byte packet, payload=%p",
                        item.dlp_flit_out),
              UVM_MEDIUM)
...
endclass
   ...
   /*task rc_check_ep_8_ts1(output bit rx_done);
      bit [1:0] rx_symbol;
      int ts1_count;
      rx_done   = 1'b0;
      ts1_count = 0;
      `uvm_info("RC_LTSSM","WAITING_FOR_8_TS1_FROM_EP",UVM_LOW)
      forever begin
         @(negedge rc_pipe_intf_rx.pclk);
         if (rc_pipe_intf_rx.rx_valid) begin
            rx_symbol = rc_pipe_intf_rx.rx_data[1:0];
            ...
            if (ts1_count >= 8) begin
               `uvm_info("RC_LTSSM","8_TS1_RECEIVED_FROM_EP",UVM_LOW)
               rx_done = 1'b1;
               break;
            end
         end
   ...
   task rc_check_ep_8_ts2(output bit rx_done);
      bit [1:0] rx_symbol;
      int ts2_count;
      rx_done   = 1'b0;
      ts2_count = 0;
      `uvm_info("RC_LTSSM","WAITING_FOR_8_TS2_FROM_EP",UVM_LOW)
      forever begin
         @(negedge rc_pipe_intf_rx.pclk);
         if (rc_pipe_intf_rx.rx_valid) begin
            rx_symbol = rc_pipe_intf_rx.rx_data[1:0];
            ...
            if (ts2_count >= 8) begin
               `uvm_info("RC_LTSSM","8_TS2_RECEIVED_FROM_EP",UVM_LOW)
               rx_done = 1'b1;
               break;
            end
         end
