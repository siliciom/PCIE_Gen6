    ...
    task receiving_pipe_tx(virtual PCIe_RC_interface rc_pipe_intf_tx);
      ...
      while (rc_pipe_intf_tx.tx_valid) begin
         ...
         if (pcie_seq_item.tx_valid) begin
            ...
            `uvm_info("TX_DATA",$sformatf("tx_data_inside_monitor_rc = %h :: %d ",tx_data,tx_data),UVM_LOW)
      ...
      if(rc_pl_model.link_up == 1 && pcie_seq_item.data_q_rc_mon_con_tx.size()> 292 ) begin 
...
`uvm_info("RC_CON_MONITOR", $sformatf(
             ...
             // 256B flit received from EP PHY -> RC controller monitor, after all
             // reverse process. FEC and CRC are computed HERE (FEC logic and CRC
             // logic each kept at one place) and the received/calculated values
             // are carried to the scoreboard. The comparison (received vs
             // calculated) and the PASS/FAIL report are done ONLY in the scoreboard.
    ...
    if(rc_pl_model.link_up==1 && pcie_seq_item.data_q_rc_mon_con_rx.size() > 292) begin
  ...
  task collect_dlp_RC_tx(output bit[0:`PCIe_DLP_BYTE_W-1][`PCIe_BYTE_W-1:0] dlp_rc_tx, ref PCIe_sequence_item item);
   ...
   do begin
	   item.data_q_rc_mon_con_tx.pop_front();
   end
   while(item.data_q_rc_mon_con_tx.size() > 61 );
  
   for(int i=0;i<`PCIe_TLP_DATA_BYTE_W;i++) begin
      word = item.data_q_rc_mon_con_tx[i/4];
      tlp_rc_tx[i] = word[8*(i%4) +: 8];
   end

   for(int i=0;i<`PCIe_DLP_BYTE_W;i++) begin
      int idx = `PCIe_TLP_DATA_BYTE_W + i;
      word = item.data_q_rc_mon_con_tx[idx/4];
      dlp_rc_tx[i] = word[8*(idx%4) +: 8];
   end
...
endtask
   ...
   bit [31:0] word;
      // remove first 232 elements in queue.
   do begin
	   item.data_q_rc_mon_con_rx.pop_front();
   end
   while(item.data_q_rc_mon_con_rx.size() > 61 );
   
     // Form tlp to send to scoreboard
      for(int i=0;i<`PCIe_TLP_DATA_BYTE_W;i++) begin
         word = item.data_q_rc_mon_con_rx[i/4];
         tlp_rc_rx[i]=word[8*(i%4) +: 8];
      end

      // Last 6 bytes = DLP
      for(int i=0;i<`PCIe_DLP_BYTE_W;i++) begin
      int idx = `PCIe_TLP_DATA_BYTE_W + i;
      word = item.data_q_rc_mon_con_rx[idx/4];
      dlp_rc_rx[i] = word[8*(idx%4) +: 8];
   end
   ...
   endtask
        ...
        `ifdef PCIE_GEN6_FEC_CRC
// ============================================================================
   // FEC LOGIC (kept in one place) - mirrors PCIe_EP_controller_monitor.sv
   // ----------------------------------------------------------------------------
   ...
   // ============================================================================
   // CRC LOGIC (kept in one place) - mirrors PCIe_EP_controller_monitor.sv
   // ----------------------------------------------------------------------------
   ...
   // ============================================================================
   // SINGLE FEC/CRC CALCULATION : 256B FLIT RECEIVED FROM EP PHY -> RC MONITOR
   // ----------------------------------------------------------------------------
   // Executed at every RX pipe dword, but only ONE full 256B flit boundary
   // (after the 232 training DWORDs) performs the check.
   ...
   task check_ep_to_rc_flit_fec_crc(input bit [`PCIe_MON_DATA_W-1:0] ep_flit_dword_q[$],
      ...
      int  flit_start;
      bit [`PCIe_BYTE_W-1:0] rc_mon_256b_flit_received_from_ep[$];   // 256B flit after reverse process
      bit [`PCIe_BYTE_W-1:0] received_242b_payload_from_ep[$];       // 242B payload of received flit
      bit [`PCIe_BYTE_W-1:0] received_8b_crc_from_ep_flit [0:7];     // 8B CRC received in the EP->RC flit
      bit [`PCIe_BYTE_W-1:0] received_6b_fec_from_ep_flit [0:5];     // 6B FEC received in the EP->RC flit
      bit [`PCIe_BYTE_W-1:0] fec_body_250b_payload_plus_ep_crc[$];   // FEC input : 242B payload + 8B received CRC
      ...
      for(int i = flit_start; i < flit_start + flit_dwords; i++) begin
         bit [31:0] dword = ep_flit_dword_q[i];
         rc_mon_256b_flit_received_from_ep.push_back(dword[7:0]);
         rc_mon_256b_flit_received_from_ep.push_back(dword[15:8]);
      ...
      for(int i = 0; i < `PCIe_DLP_FLIT_BYTE_W; i++)
         received_242b_payload_from_ep.push_back(rc_mon_256b_flit_received_from_ep[i]);
      for(int i = 0; i < `PCIe_FLIT_CRC_BYTES; i++)
         received_8b_crc_from_ep_flit[i] = rc_mon_256b_flit_received_from_ep[`PCIe_DLP_FLIT_BYTE_W + i];
      ...
      pcie_seq_item.ep_to_rc_flit_received_8b_crc           = {received_8b_crc_from_ep_flit[0], received_8b_crc_from_ep_flit[1],
                                                         received_8b_crc_from_ep_flit[2], received_8b_crc_from_ep_flit[3],
