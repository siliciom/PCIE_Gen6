...
class PCIe_RC_DL_model extends uvm_component;
  ...
  `uvm_component_utils(PCIe_RC_DL_model)
    ...
    bit[`PCIe_SEQ_NUM_W-1:0]     TX_ACKNAK_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_MAX;
    bit[`PCIe_SEQ_NUM_W-1:0]     NEXT_TX_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_ZERO + 1;
    bit[`PCIe_SEQ_NUM_W-1:0]     NEXT_EXPECTED_RX_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_ZERO + 1;
    bit[`PCIe_SEQ_NUM_W-1:0]     IMPLICIT_RX_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_ZERO;
    bit[`PCIe_SEQ_NUM_W-1:0]     ACKD_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_MAX; 
    bit [`PCIe_SEQ_NUM_W-1:0]    TX_REPLAY_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_ZERO; 
    bit [`PCIe_SEQ_NUM_W-1:0]    NAK_IGNORE_FLIT_SEQ_NUM=`PCIe_SEQ_NUM_ZERO;
    bit          NON_IDLE_EXPLICIT_SEQ_NUM_FLIT_RCVD;  
    bit [`PCIe_FLIT_REPLAY_NUM_W-1:0]    FLIT_REPLAY_NUM=`PCIe_FLIT_REPLAY_NUM; 
    bit          REPLAY_SCHEDULED; 
    int unsigned MAX_UNACKNOWLEDGED_FLITS = `PCIe_MAX_UNACK_FLITS; 

    bit [`PCIe_SEQ_NUM_W-1:0]    RX_RETRY_BUFFER_LAST_FLIT_SEQ_NUM; // Stores ID of last flit before overflow
    bit [`PCIe_SEQ_NUM_W-1:0]    NEXT_RX_FLIT_SEQ_NUM_TO_STORE;     // ID of next flit to go in buffer
...
// FLIT MODE PACKET CREATION DLP
    task  form_dl_packet(
    input  bit [0:`PCIe_TLP_DATA_BYTE_W-1][`PCIe_BYTE_W-1:0] tlp_data,       // Incoming 236 bytes from TL
    input  bit               is_payload,     // 1=Payload flit, 0=NOP/IDLE flit
    output bit [0:`PCIe_DLP_FLIT_BYTE_W-1][`PCIe_BYTE_W-1:0] dl_flit_out     // 242-byte output (TLPs + DLP)
);
    ...
    //`uvm_info("RC_DL_MODEL",$sformatf("is_payload=%h :: dllp_sequence_number=%0h :: dlp=%p",is_payload,{dlp[0][1:0], dlp[1]},dl_flit_out),UVM_LOW);
    `uvm_info("RC_DL_MODEL",$sformatf("replay_cmd=%d :: is_payload=%d :: dllp_sequence_number=%0h :: dlp=%p",replay_cmd,is_payload,{dlp[0][1:0], dlp[1]},dl_flit_out),UVM_LOW);
    store_tx_retry_buffer(tlp_data,seq_num_to_send);
   endtask

  // create nop2 dllp
  task create_no2_dllp(output dllp_content);
	  dllp_content=0;
  endtask
...
// Handling the incoming flit and passing info to RC
  task handle_incoming_flit(bit[5:0][7:0]dlp, bit is_valid);
    ...
    //`uvm_info("RC_DL_MODEL",$sformatf("DLP=%p, valid=%d, sequence_number=%d, is_nop=%d, prior_was_payload=%d,is_idle=%d,is_explicit=%d",dlp,is_valid,sequence_number,is_nop,prior_was_payload,is_idle,is_explicit),UVM_LOW);

    update_implicit_rx_sequence_number(sequence_number,is_valid,is_nop,is_payload,is_explicit,is_idle,prior_was_payload);
    
    //`uvm_info("RC_DL_MODEL",$sformatf("NEXT_TX_FLIT_SEQ_NUM = %h, NEXT_EXPECTED_RX_FLIT_SEQ_NUM = %h,IMPLICIT_RX_FLIT_SEQ_NUM = %h,ACKD_FLIT_SEQ_NUM = %h",NEXT_TX_FLIT_SEQ_NUM,NEXT_EXPECTED_RX_FLIT_SEQ_NUM,IMPLICIT_RX_FLIT_SEQ_NUM,ACKD_FLIT_SEQ_NUM),UVM_LOW);
...
/***********************************************************************************************/
 // 2. TX logic handling
	...
	begin
       ...
       //`uvm_info("RC_DL_MODEL",$sformatf("N=%d :: is_nak=%d :: is_ack=%d", N,is_nak,is_ack),UVM_LOW);
...
//`uvm_info("RC_DL_MODEL",$sformatf("The content of the retry buffer is %p",tx_retry_buffer),UVM_LOW);
endtask
...
endtask
  ...
  function void write(PCIe_sequence_item item);
   ...
   //`uvm_info("RC_DL_MODEL",$sformatf("DL -> PL: replayed_item_from_driver=%p", item.tlp_data),UVM_LOW)
