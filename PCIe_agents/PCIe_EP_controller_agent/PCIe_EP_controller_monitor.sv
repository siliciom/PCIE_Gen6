...
class PCIe_EP_controller_monitor extends uvm_monitor;
  ...
  uvm_analysis_port #(PCIe_sequence_item) ep_ap_mon_dl;
  ...
  bit tx_parity;
  bit rx_parity;
  bit [`PCIe_PL_PIPE_WORD_W-1:0] dl_flit_in[$];
  ...
  task receiving_pipe_rx(virtual PCIe_EP_interface ep_pipe_intf_rx);
    ...
    bit [`PCIe_PL_PIPE_WORD_W-1:0] deprecoded_data;
    bit [`PCIe_PL_PIPE_WORD_W-1:0] gray_decoded_data;
    bit [`PCIe_PL_PIPE_WORD_W-1:0] descrambled_data;
    ...
    if(ep_dl_model.success) begin
      ...
      `uvm_info("EP_CON_MONITOR","The DLP_packet_sent_to_scoreboard",UVM_LOW)
    end
  ...
  task de_precoder_rx(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in,output bit [`PCIe_PL_PIPE_WORD_W-1:0] decoder_out);
    bit [`PCIe_PL_SYMBOL_W-1:0] current;
    bit [`PCIe_PL_SYMBOL_W-1:0] dec;

    `uvm_info("EP_CONTROLLER","ENTERED_INTO_EP_CONTROLLER_MONITOR_DE_PRECODE_TASK",UVM_LOW)

    for(int i=0;i<`PCIe_PL_PIPE_WORD_W;i+=`PCIe_PL_SYMBOL_W) begin
  ...
  task gray_decode_rx(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in,output bit [`PCIe_PL_PIPE_WORD_W-1:0] gray_out);
  ...
  task descrambler_rx(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in,output bit [`PCIe_PL_PIPE_WORD_W-1:0] data_out);
    bit descramble_bit;
  ...
  task de_precoder_tx(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in,output bit [`PCIe_PL_PIPE_WORD_W-1:0] decoder_out);
    bit [`PCIe_PL_SYMBOL_W-1:0] current;
    bit [`PCIe_PL_SYMBOL_W-1:0] dec;
  ...
  task gray_decode_tx(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in,output bit [`PCIe_PL_PIPE_WORD_W-1:0] gray_out);
    bit [`PCIe_PL_SYMBOL_W-1:0] symbol;
  ...
  task descrambler_tx(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in,output bit [`PCIe_PL_PIPE_WORD_W-1:0] data_out);
    bit descramble_bit;
  ...
  task collect_dlp(output bit [0:`PCIe_DLP_BYTE_W-1][`PCIe_BYTE_W-1:0] dlp,ref PCIe_sequence_item item);
    ...
    for(int i=0;i<`PCIe_DLP_BYTE_W;i++) begin
      dlp[i]=item.data_q_ep_mon_con_rx[`PCIe_TLP_DATA_BYTE_W+i];
    end
