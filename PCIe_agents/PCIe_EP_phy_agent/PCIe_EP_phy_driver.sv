...
class PCIe_EP_phy_driver extends uvm_driver #(PCIe_sequence_item);
    ...
    forever begin
      for(int i=0;i<`PCIe_PL_PIPE_WORD_W;i++) begin
        ...
        `uvm_info("EP_PHY_DRIVER",
                  $sformatf("RX_BIT[%0d]=%0b TIME=%0.5f",
                            i,rx_bit,$realtime),
                  UVM_LOW)
      end
      ...
      `uvm_info("EP_PHY_DRIVER",
                $sformatf("RX_32_BIT_PARALLEL_DATA=%08h QUEUE_SIZE=%0d",
                          rx_data,rc_data_q.size()),
                UVM_LOW)
    end
  ...
  task sending_pipe_rx(virtual PCIe_EP_interface ep_pipe_intf_rx);
    ...
    `uvm_info("EP_PHY_DRIVER",
              "ENTERED_INTO_SENDING_PARALLEL_DATA_TO_PIPE_RX",
              UVM_LOW)

    ep_pipe_intf_rx.phy_status<=1'b0;
    ep_pipe_intf_rx.rx_elec_idle<=1'b0;
    ep_pipe_intf_rx.rx_status<='0;
    ...
    forever begin
      ...
      if(rc_data_q.size()>0) begin
        ...
        `uvm_info("EP_PHY_DRIVER",
                  $sformatf("DRIVING_EP_PIPE_RX_DATA=%08h",
                            rx_parallel_data),
                  UVM_LOW)
      end
      else begin
      ...
      `uvm_info("EP_PHY_DRIVER",
                "EXIT_FROM_SENDING_PARALLEL_DATA_TO_PIPE_RX",
                UVM_LOW)
    end
  endtask
  ...
  task receiving_data_ep(virtual PCIe_EP_interface ep_pipe_intf_tx);
    ...
    `uvm_info("EP_PHY_DRIVER",
              "ENTERED_INTO_RECEIVING_PIPE_DATA_EP_TASK",
              UVM_LOW)

    wait(ep_pipe_intf_tx.tx_valid);

    `uvm_info("EP_PHY_DRIVER",
              "STARTED_RECEIVING_PIPE_DATA",
              UVM_LOW)
  ...
  task tx_piso(input bit [`PCIe_PL_PIPE_WORD_W-1:0] data_in);
    ...
    for(int i=0;i<`PCIe_PL_PIPE_WORD_W;i++) begin
      ...
      `uvm_info("EP_PHY_DRIVER",
                $sformatf("EP_PHY_DRIVER_PISO_DATA_IN_PHY_BIT_BY_BIT[%0d]=%0b TIME=%0t",
